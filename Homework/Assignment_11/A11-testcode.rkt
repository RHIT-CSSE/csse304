...
(define test (make-test ; (r)
  (my-let equal? ; (run-test my-let)
    ...
    [(my-let ((a 5)) (+ 3 (my-let fact ((n a)) (if (zero? n) 1 (* n (fact (- n 1))))))) 123 1] ; (run-test my-let 3)
    [(my-let ((a (lambda () 3))) (my-let ((a (lambda () 5)) (b a)) (b))) 3 1] ; (run-test my-let 4)
  )

  (my-or equal? ; (run-test my-or)
    [(begin (set! a #f) (my-or #f (begin (set! a (not a)) a) #f)) #t 1] ; (run-test my-or 1)
    ...
    [(my-or) #f 1] ; (run-test my-or 4)
    [(let ((x 0)) (if (my-or #f 4 (begin (set! x 12) #t)) (set! x (+ x 1)) (set! x (+ x 3))) x) 1 1] ; (run-test my-or 5)
    [(my-or #f 4 3) 4 1] ; (run-test my-or 6)
    [(let ((x 0)) (my-or (begin (set! x (+ 1 x)) x) #f)) 1 1] ; (run-test my-or 7)
    [(my-or 6) 6 1] ; (run-test my-or 8)
  )

  (+= equal? ; (run-test +=)
    [(let ((a 5)) (+= a 10) (+ a 10)) 25 1] ; (run-test += 1)
    [(begin (let* ((a 10) (b 21) (c (+= a (+= b a)))) (list a b c))) '(41 31 41) 1] ; (run-test += 2)
  )

  (return-first equal? ; (run-test return-first)
    [(return-first 2) 2 1] ; (run-test return-first 1)
    [(begin (let ((a 3)) (return-first (+ a 2) (set! a 7) a))) 5 1] ; (run-test return-first 2)
    [(return-first (return-first 3 4 5) 1 2) 3 1] ; (run-test return-first 3)
  ...
  (parse-unparse equal? ; (run-test parse-unparse)
    ...
    [(unparse-exp (parse-exp (quote (lambda () (letrec ((foo (lambda (L) (if (null? L) 3 (if (symbol? (car L)) (cons (car L) (foo (cdr L))) (foo (cdr L))))))) foo))))) '(lambda () (letrec ((foo (lambda (L) (if (null? L) 3 (if (symbol? (car L)) (cons (car L) (foo (cdr L))) (foo (cdr L))))))) foo)) 1] ; (run-test parse-unparse 10)
    ...
    [(unparse-exp (parse-exp (quote (lambda (c) (if (char? c) string 12345))))) '(lambda (c) (if (char? c) string 12345)) 1] ; (run-test parse-unparse 13)
    [(unparse-exp (parse-exp (quote (lambda (datum) (or (number? datum) (boolean? datum) (null? datum) (string? datum) (symbol? datum) (pair? datum) (vector? datum)))))) '(lambda (datum) (or (number? datum) (boolean? datum) (null? datum) (string? datum) (symbol? datum) (pair? datum) (vector? datum))) 1] ; (run-test parse-unparse 14)
    [(unparse-exp (parse-exp (quote (lambda (t) (let ((L (build-list t)) (sum< (lambda (a b) (< (cdr a) (cdr b)))) (intsum? (lambda (x) (symbol? (car x))))) (car (genmax sum< (filter intsum? L)))))))) '(lambda (t) (let ((L (build-list t)) (sum< (lambda (a b) (< (cdr a) (cdr b)))) (intsum? (lambda (x) (symbol? (car x))))) (car (genmax sum< (filter intsum? L))))) 1] ; (run-test parse-unparse 15)
    [(unparse-exp (parse-exp (quote (letrec ((a (lambda () (b 2))) (b (lambda (x) (- x 4)))) (lambda () (a)))))) '(letrec ((a (lambda () (b 2))) (b (lambda (x) (- x 4)))) (lambda () (a))) 1] ; (run-test parse-unparse 16)
    [(unparse-exp (parse-exp (quote (let* ((a (lambda () (c 4))) (b a)) (lambda (b) (a)))))) '(let* ((a (lambda () (c 4))) (b a)) (lambda (b) (a))) 1] ; (run-test parse-unparse 17)
    [(unparse-exp (parse-exp (quote (lambda x (cons a x))))) '(lambda x (cons a x)) 1] ; (run-test parse-unparse 18)
    [(unparse-exp (parse-exp (quote (lambda x (let* ((a x) (b (cons x a))) b))))) '(lambda x (let* ((a x) (b (cons x a))) b)) 1] ; (run-test parse-unparse 19)
    [(unparse-exp (parse-exp (quote (lambda (a b c) (let* ((dbl (lambda x (append x x))) (lst (dbl a b c))) lst))))) '(lambda (a b c) (let* ((dbl (lambda x (append x x))) (lst (dbl a b c))) lst)) 1] ; (run-test parse-unparse 20)
    [(unparse-exp (parse-exp (quote (lambda a (letrec ((stuff (lambda (b) (if (null? b) (list) (cons (car b) (stuff (cdr b))))))) (stuff a)))))) '(lambda a (letrec ((stuff (lambda (b) (if (null? b) (list) (cons (car b) (stuff (cdr b))))))) (stuff a))) 1] ; (run-test parse-unparse 21)
    [(unparse-exp (parse-exp (quote (lambda (a b c) (let* ((a b) (d (append a c))) d))))) '(lambda (a b c) (let* ((a b) (d (append a c))) d)) 1] ; (run-test parse-unparse 22)
  )
))
