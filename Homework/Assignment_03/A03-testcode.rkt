...
(define test (make-test
  (intersection set-equals?
    [(intersection '(a b d e f h i j) '(h q r i z)) '(i h) 1]
    [(intersection '(g h i) '(j k l)) '() 1]
    [(intersection '(a p t) '()) '() 1]
    [(intersection '() '(g e t)) '() 1]
  )

  (subset? equal?
    [(subset? '(c b) '(a d b e c)) #t 1]
    [(subset? '(c b) '(a d b e)) #f 1]
    [(subset? '(c b) '()) #f 1]
    [(subset? '(c b) '(b c)) #t 1]
  ...
  (relation? equal?
    [(relation? 5) #f 1]
    [(relation? '()) #t 1]
    [(relation? '((a b) (b c))) #t 1]
    [(relation? '((a b) (b a) (a a) (b b))) #t 1]
    [(relation? '((a b) (b c d))) #f 1]
    [(relation? '((a b) (c d) (a b))) #f 1]
    [(relation? '((a b) (c d) "5")) #f 1]
    [(relation? '((a b) . (b c))) #f 1]
  )

  (domain set-equals?
    [(domain '((1 2) (3 4) (1 3) (2 7) (1 6))) '(2 3 1) 1]
    [(domain '()) '() 1]
  )

  (reflexive? eq?
    [(reflexive? '((a a) (b b) (c d) (b c) (c c) (e e) (c a) (d d))) #t 1]
    [(not (reflexive? '((a a) (b b) (c d) (b c) (e e) (c a) (d d)))) #t 1]
    [(not (reflexive? '((a a) (c d) (b c) (c c) (e e) (c a) (d d)))) #t 1]
    [(reflexive? '()) #t 1]
    [(not (reflexive? '((c c) (b b) (c d) (b c) (e e) (c a) (d d)))) #t 1]
  )

  (multi-set? equal?
    [(and (multi-set? '()) (not (multi-set? '(a b)))) #t 1]
