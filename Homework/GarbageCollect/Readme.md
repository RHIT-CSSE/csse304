...
If we look at memory after these to declarations we see:

    '#(3 -1 222 3 0 111 -1 -1 -1 -1)

3 is the type of a linked-list-node, which are always 3 integers long
...
The the ptr struct you see being bandied about is just a little scheme
...
consider this special case in your code and not try to say access it
like a valid object.  If you need to create a null-ptr in your own
code, null-ptr is prebuilt one you can use.  If you want to test to
see if a value is null null-ptr? is a function to use.
...
our case we're happy to just walk memory.
...
Instead, what we'll need to do is compact our memory to garbage
...
code that uses them does not realize memory has been adjusted.
...
* Once the accessible structs are calculated, its easy given their
...
* I encourage you to test with you only test structs and cases by hand
...
clear to you how those features could be added.  Feel free to play
...
If you look at the functions for evaluation, you will see that none of
them take as parameters and none of them return anything but memstruct
types and raw integers.
...
referencing the expression, environment, and continuation.
