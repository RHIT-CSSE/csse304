...
(define test (make-test ; (r)
  ...
  (primitive-procedures equal? ; (run-test primitive-procedures)
    ...
    [(list (eval-one-exp '(symbol? 'a)) (eval-one-exp '(symbol? 5))) '(#t #f) 1] ; (run-test primitive-procedures 22)
  ...
  (let equal? ; (run-test let)
    [(eval-one-exp ' (let ((a 3)(b 5)) (+ a b))) 8 1] ; (run-test let 1)
    [(eval-one-exp ' (let ((a 3)) (let ((b 2) (c (+ a 3)) (a (+ a a))) (+ a b c)))) 14 1] ; (run-test let 2)
    [(eval-one-exp ' (let ((a 3)) (let ((a (let ((a (+ a a))) (+ a a)))) (+ a a)))) 24 1] ; (run-test let 3)
    [(eval-one-exp ' (let ((a 3) (b 5)) (set! a 2) (set! b 4) (cons a b))) '(2 . 4) 1] ; (run-test let 4)
  )

  (lambda equal? ; (run-test lambda)
    [(eval-one-exp '((lambda (x) (+ 1 x)) 5)) 6 1] ; (run-test lambda 1)
    [(eval-one-exp '((lambda (x) (+ 1 x) (+ 2 (* 2 x))) 5)) 12 1] ; (run-test lambda 2)
    [(eval-one-exp ' ((lambda (a b) (let ((a (+ a b)) (b (- a b))) (let ((f (lambda (a) (+ a b)))) (f (+ 3 a b))))) 56 17)) 154 1] ; (run-test lambda 3)
    [(eval-one-exp ' (((lambda (f) ((lambda (x) (f (lambda (y) ((x x) y)))) (lambda (x) (f (lambda (y) ((x x) y)))))) (lambda (g) (lambda (n) (if (zero? n) 1 (* n (g (- n 1))))))) 6)) 720 1] ; (run-test lambda 4)
    [(eval-one-exp ' (let ((Y (lambda (f) ((lambda (x) (f (lambda (y) ((x x) y)))) (lambda (x) (f (lambda (y) ((x x) y))))))) (H (lambda (g) (lambda (x) (if (null? x) '() (cons (procedure? (car x)) (g (cdr x)))))))) ((Y H) (list list (lambda (x) x) 'list)))) '(#t #t #f) 1] ; (run-test lambda 5)
  )
))
