...
(define all-symbols  ; Find all symbols in the domain and range.
  (lambda (rel)      ; Perhaps it could be slightly more efficient, but 
    (if (null? rel)  ; any approach will be O(N^2).
	'()
	(union (list (caar rel)) (union (list (cadar rel)) (all-symbols (cdr rel)))))))
...
(define k) 
(define v)
(define rel-ref)    ; relation variable fo "argument" to reflexive?-imp
(define rel-all)    ; relation variable fo "argument" to all-syms-imp
(define rel-pairs)  ; relation variable fo "argument" to reflexive-pairs-imp
...
(define apply-k-imp
  (lambda ()
    ...
    (cases kontinuation k
	   [init-k () v] ; v is final enswer
	   [list-k () (list v)]
	   [cdr-union-k (car-s1 s2  k1)
	      (set! sym-memq car-s1)
	      (set! ls s2)
	      (set! k (union-memq-k car-s1 v k1))
	      (memq-imp)]
	   [union-memq-k (car-s1 cdr-union k1)
	      (set! v (if v
			  cdr-union
			  (cons car-s1 cdr-union)))
	      (set! k k1)
	   ...
	   [all-syms-k (car-rel k1)
	      ...
	      (set! s2 v)
	      (set! k (all-union-k (car car-rel) k1))
	      (union-imp)]
	   [all-union-k (caar k1)
	      (set! s1 (list caar))
	      (set! s2 v)
...
(define all-syms-imp
  (lambda ()
	...
	(begin (set! v '())
	       (apply-k-imp))
	(begin (set! k (all-syms-k (car rel-all) k))
...
(test '((a a)))
(test '((a a) (b a) (b b)))
(test '((a b) (b c) (c d) (a a) (b b) (c c)))
(test '((a b) (b c) (c d) (a a) (b b) (c c) (d d)))
