  ...
  (lambda (exp)
    (cond
      [(null? exp) 'empty-list]
       ...
       [(number? exp) 'number]
       [(eqv? (car exp) 'cons)
        (let ((tcar (cons-type (second exp)))
              (tcdr (cons-type (third exp))))
          (cond [(eqv? tcdr 'empty-list) (list tcar)]
                [(not (list? tcdr)) (raise 'cons-nonlist)]
                [(not (equal? tcar (car tcdr))) (raise 'list-mismatch-cons)]
                [else tcdr]))]
       [(eqv? (car exp) 'cdr)
        (let ((tsub (cons-type (second exp))))
          (if (list? tsub) tsub (raise 'cdr-nonlist)))]
       [(eqv? (car exp) 'car)
        (let ((tsub (cons-type (second exp))))
          (if (list? tsub) (car tsub) (raise 'car-nonlist)))])))
