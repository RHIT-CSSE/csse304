...
(define-datatype continuation continuation?
  [init-k]
  [list-k]
  [car-symbol (slist list?) (old symbol?) (new symbol?) (k continuation?)]
  [inner (recur-car list?) (k continuation?)]
  [outer (slist list?) (old symbol?) (new symbol?) (k continuation?)]
  )
...
(define apply-k
  (lambda (k v)
    (cases continuation k
      [init-k () v]
      [list-k () (list v)]
      [car-symbol (slist old new k1)
                  (apply-k k1 (cons (if (eqv? (car slist) old) new (car slist)) v))]
      [inner (recur-car k2) 
             (apply-k k2 (cons recur-car v))]
      [outer (slist old new k3)
             (slist-subst-datatype (cdr slist) old new (inner v k3))])))

(define slist-subst-datatype
  (lambda (slist old new k)
    (cond [(null? slist) (apply-k k '())]
          [(symbol? (car slist))
           (slist-subst-datatype (cdr slist) old new (car-symbol slist old new k))]
                   
          [else (slist-subst-datatype (car slist) old new (outer slist old new k))])))
...
; I don't have a full solution because I can't reveal my interpreter
; here's the main part

          [qlet-exp (bodies)
                    (cases expression (car bodies)
                      (qset-exp (name valueexp)
                            (expand-exp (let-exp 'let (list name) (list valueexp)
                                                 (list (if (null? (cdr bodies))
                                                     (app-exp (list (var-exp 'void)))
                                                     (qlet-exp (cdr bodies)))))))
                      (else
                       (expand-exp (if (null? (cdr bodies))
                                       (car bodies)
                                       (begin-exp (list (car bodies)
                                                        (qlet-exp (cdr bodies))))))))]
...
;; global).
;;
;; (var-prefic aa (let ((aax 1)) x)) ; ERROR!
...
; again I don't want to reveal my full interpreter
;
; my solution rests on creating a new type of envrionment when the var
; prefix expression is found.  Then my apply-env entry looks like this:
;
; note I'm using I pass a lambda to apply-env which is invoked if the
; lookup fails

      [var-prefix-record (prefix env)
                         (apply-env env sym (lambda ()
                                              (apply-env env (append-sym prefix sym) (lambda () (apply-env global-env (append-sym prefix sym) on-failure)))))]
